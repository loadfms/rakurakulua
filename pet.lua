...
function Pet:load()
    ...
    self.y = wh - 30 - (self.spriteHeight * self.scale)
...
function Pet:update(dt)
    local buttonsWidth = (self.spriteWidth * 2)
...
function Pet:draw()
    self.anim:draw(self.spriteSheet, self.x, self.y, nil, self.scale)
end
...
function Pet:feed()
    self.state = self.state == "sleeping" and "idle" or "sleeping"
end

function Pet:toggleLights()
